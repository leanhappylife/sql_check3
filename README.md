...
This folder contains the final DB2 sample scripts and expected outputs.

Rules:
- one table per file under table/
- one view per file under view/
- one function per file under function/
- one procedure per file under sp/
- extra/ contains non-object pattern coverage
- expected/ contains:
  - relationship_detail.tsv
  - lineage_path.tsv

DB2 terminator rule:
- top-level object scripts use `@` on its own line
- internal routine statements still use `;`

Important:
- relationship_detail.tsv stores direct single-hop relationships only
- lineage_path.tsv stores propagated end-to-end paths
- INSERT_TARGET_COL may be inferred from metadata when target column list is omitted
- line_content in relationship_detail.tsv is expected to match raw sample source lines, except metadata-inferred rows


This package uses the fully verified relationship_detail.tsv.
...
---------------
--versionPlease extend the existing Java project incrementally.
