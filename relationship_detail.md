...
`persistent_impact_objects` and `intermediate_objects` are auxiliary tracking columns used to categorize the target's persistence lifecycle. They do **not** change the semantic meaning of the row.
...
Auxiliary tracking column for mappings that land in permanent database storage.

Rules:
- Populate this column only if the `target_object_type` is a permanent entity (`TABLE`, `VIEW`, or `UNKNOWN` representing a persistent target).
- Format:
  - `target_object.target_field` when `target_field` is present
  - otherwise `target_object`

### `intermediate_objects`
Auxiliary tracking column for mappings that land in temporary, in-memory, or pass-through structures.

Rules:
- Populate this column only if the `target_object_type` is a temporary or intermediate entity (`SESSION_TABLE`, `CTE`, `VARIABLE`, `CURSOR`, or `PROCEDURE` / `FUNCTION` when representing parameter assignments).
- Format:
  - `target_object.target_field` when `target_field` is present
  - otherwise `target_object`
